...
function ReportContent() {
    ...
    // For the history list view (no session ID)
    const [historyList, setHistoryList] = useState([])
    const [historyLoading, setHistoryLoading] = useState(true)

    useEffect(() => {
        if (!sessionId) {
            // Show user report history instead of redirecting
    ...
    const getStressBadge = (score) => {
        ...
        if (n < 40) return { label: 'Calm', cls: 'bg-emerald-100 text-emerald-700 border-emerald-200' }
        if (n < 70) return { label: 'Moderate', cls: 'bg-amber-100 text-amber-700 border-amber-200' }
        return { label: 'High', cls: 'bg-red-100 text-red-700 border-red-200' }
    }

    // ── History list view (no ?session= param) ──────────────────────────────
    if (!sessionId) {
        return (
            <div className="space-y-6 max-w-4xl mx-auto animate-in fade-in duration-500">
                <div>
                    ...
                    <h2 className="text-3xl font-extrabold text-white">Stress Reports</h2>
    ...
    // ── Single report view (has ?session= param) ────────────────────────────
    if (loading) {
        return (
